...
async function callOllamaChat(body) {
    ...
    try {
        ...
        // normalize network/connection errors so callers can handle them safely
...
app.post('/api/chat/init', express_1.default.json(), async (req, res) => {
    try {
        ...
        try {
            ...
            // If chat endpoint failed, try generate. If both fail, return 502 with helpful message.
            try {
                resp = await callOllamaGenerate(body);
            }
            catch (e2) {
                console.error('[api/chat/init] ollama call failed', e, e2);
                const msg = (e && e.code === 'OLLAMA_CONNECTION_FAILED') || (e2 && e2.code === 'OLLAMA_CONNECTION_FAILED')
                    ? 'OLLAMA_UNREACHABLE'
                    : 'OLLAMA_ERROR';
                const detail = (e && e.message) || (e2 && e2.message) || undefined;
